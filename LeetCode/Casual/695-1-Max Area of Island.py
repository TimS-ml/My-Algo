...
class Solution:
    ...
    def dfs(self, row, col, grid, seen):
        ...
        if 0 <= row < len(grid) and 0 <= col < len(
                grid[0]) and (grid[row][col] == 1) and (row, col) not in seen:
            # list this point to seen set so we won't count it again
            seen.add((row, col))
            return (self.dfs(row - 1, col, grid, seen) +
                    self.dfs(row + 1, col, grid, seen) +
                    self.dfs(row, col - 1, grid, seen) +
                    self.dfs(row, col + 1, grid, seen) + 1
                    )  # add 1 to the area and DFS(4-Conn Neighbors)
        else:
